...
void QWorkQueue::init() {
  ...
  done_queue = new boost::lockfree::queue<TxnManager*>(0);
...
void QWorkQueue::sequencer_enqueue(uint64_t thd_id, Message* msg) {
  ...
  work_queue_entry* entry =
      (work_queue_entry*)mem_allocator.align_alloc(sizeof(work_queue_entry));
  entry->msg = msg;
...
void QWorkQueue::sched_enqueue(uint64_t thd_id, Message* msg) {
  ...
  work_queue_entry* entry =
      (work_queue_entry*)mem_allocator.alloc(sizeof(work_queue_entry));
  entry->msg = msg;
  entry->rtype = msg->rtype;
  entry->txn_id = msg->txn_id;
  entry->batch_id = msg->batch_id;
  ...
  while (!sched_queue[msg->get_return_id()]->push(entry) &&
         !simulation->is_done()) {
  }
  ...
  if (valid) {
    ...
    if (msg->rtype == RDONE) {
      ...
      if (sched_ptr == g_node_cnt - 1) {
        ...
        INC_STATS(thd_id, sched_epoch_diff,
                  get_sys_clock() - simulation->last_worker_epoch_time);
      ...
      DEBUG("Sched msg dequeue %ld (%ld,%ld) %ld\n", sched_ptr, msg->txn_id,
            msg->batch_id, simulation->get_worker_epoch());
